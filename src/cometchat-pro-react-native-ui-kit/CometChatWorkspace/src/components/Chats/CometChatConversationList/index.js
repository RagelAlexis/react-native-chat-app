...
class CometChatConversationList extends React.Component {
  ...
  componentDidMount() {
    this.decoratorMessage = 'Chargement...';
      ...
      if (
        ...
        if (groupKey > -1) {
          ...
          if (conversationList.length === 0) {
            this.decoratorMessage = 'Aucun chat trouvé';
          }
        }
      }
    } catch (error) {
    ...
    return (
      <View style={[styles.conversationHeaderStyle]}>
        <View style={styles.headingContainer}>
          <Text style={styles.conversationHeaderTitleStyle}>Messages</Text>
        </View>
      </View>
    );
  };
