...
export default (props) => {
  ...
  const roles = {};
  roles[CometChat.GROUP_MEMBER_SCOPE.ADMIN] = 'Administrateur';
  roles[CometChat.GROUP_MEMBER_SCOPE.MODERATOR] = 'Modérateur';
    ...
    changeScope = (
      <View style={style.changeScopeContainer2}>
        ...
        <TouchableOpacity onPress={() => toggleChange(true)}>
          {scopeIcon}
          {scopeIcon ? <Text style={style.editText}>Éditer</Text> : null}
        </TouchableOpacity>
      </View>
    );
  }
  ...
  if (props.item.owner === props.member.uid) {
    receivedScope = <Text style={style.ownerText}>Propriétaire</Text>;
    changeScope = receivedScope;
    ban = null;
    kick = null;
  }
    ...
    editAccess = (
      <View style={style.editAccessContainer}>
        <TouchableOpacity
          ...
          {ban}
          {ban ? <Text style={style.actionText}>Bannir</Text> : null}
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => {
          ...
          {kick}
          {kick ? <Text style={style.actionText}>Supp</Text> : null}
        </TouchableOpacity>
      </View>
    );
  }
